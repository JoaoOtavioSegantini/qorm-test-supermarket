...
func SetupWidget(Admin *admin.Admin) {
	...
	Widgets = widget.New(&widget.Config{DB: database.DB})
	...
	Widgets.RegisterWidget(&widget.Widget{
		...
		Context: func(context *widget.Context, setting interface{}) *widget.Context {
			if setting != nil {
				var products []*models.Product
				context.GetDB().Limit(9).Preload("Categoria").Where("id IN (?)", setting.(*selectedProductsArgument).Products).Find(&products)
